 ...
 * un control XForm Upload.  
 *
 * @Author      Mauricio Rodriguez
 * @Versión     1.0, 25/01/2005
 */

public class UploadControl extends JControl{

	private JTextField jTextField;			//representa a un control input.
	 ...
	 * 						   control.
	 */
	public UploadControl(DocTable docTable, DocTree docTree, XmlDoc xmldoc, JEditPanel editPanel) {
		super(editPanel);
		...
		jTextField.setLocation((ANCHO_EXTRA / 2), ALTO_EXTRA / 2);
		uploadButton.setBounds(jTextField.getWidth() + (ANCHO_EXTRA / 2) + 3, ALTO_EXTRA / 2, 80, ALTO);
		this.setLayout(null);
		this.addActionListener(actionEvent);
		this.add(actionEvent, null);
		this.add(jTextField, null);
		this.add(uploadButton, null);
		this.properties = new Properties(this.jTable,this.table);
		this.setControlType("upload");
    }
	 ...
	 */
	public UploadControl(DocTable docTable, DocTree docTree, Element repeatControl, JEditPanel editPanel) {
		super(editPanel);
    	this.docTree = docTree;
    	this.docTable = docTable;
    	this.table = docTable.getTable();
	 ...
	 * 						   control.
	 */
    public UploadControl(int CONTROL_X, int X1, int Y1, int X2, int Y2, DocTable docTable, 
		...
		this.jTable = docTable.getJTable();
		this.xmldoc = xmldoc;
		this.uploadButton = new JButton("Buscar");
		this.jTextField = new JTextField("");
    	this.jTextField.setEnabled(false);
    	this.actionEvent.setBounds(ANCHO_EXTRA / 2, ALTO_EXTRA / 2, X2 - X1, ALTO);
		this.jTextField.setBounds(ANCHO_EXTRA / 2, ALTO_EXTRA / 2, (X2 - X1) - 83, ALTO);
		this.uploadButton.setBounds(jTextField.getWidth() + (ANCHO_EXTRA / 2) + 3, ALTO_EXTRA / 2, 80, ALTO);
		this.setLayout(null);
		this.addActionListener(actionEvent);
		...
		this.properties = new Properties(this.jTable, this.table);
		this.setControlType("upload");
		if(CONTROL_X != 0) 	this.setName("Upload_"+CONTROL_X);
		else this.setName("Upload");
		this.updateControl();
		shapesVisible(false);
    }
	...
	public void reDraw() {
		rect.setVisible(false);
		jTextField.setSize((getWidth() - +ANCHO_EXTRA) - 83, ALTO);
		uploadButton.setLocation(jTextField.getWidth() + (ANCHO_EXTRA / 2) + 3, ALTO_EXTRA / 2);
